...
interface Props {
  ...
  handleCommentSubmit: (e: React.FormEvent<HTMLFormElement>, id: string) => void;
...
const VideoFeedItem: React.FC<Props> = ({
  ...
  const commentPreview = videoData?.comments?.[0]
    ? `${videoData.comments[0].author?.displayName || videoData.comments[0].author?.handle || ""}: ${videoData.comments[0].content}`
    : undefined;
  const isLoaded = !!videoData?.url;
  const displayedCommentCount = videoCommentCounts[id] ?? (videoData?.comments?.length || 0);
  ...
  return (
    ...
    >
      <div
        ...
        style={{ aspectRatio: "9 / 16", maxHeight: "100vh", maxWidth: "calc(100vh * 9 / 16)", width: "100%" }}
      >
        {isLoaded ? (
          <video
            data-index={id}
          ...
          <div className="absolute left-3 bottom-4 right-28 flex flex-col gap-1 pointer-events-none z-10">
            {videoData?.author && typeof videoData.author === "object" && (
              <span className="font-semibold text-sm drop-shadow">
                {videoData.author.displayName || videoData.author.handle || "User"}
              </span>
            )}
            {videoData?.description && <span className="text-xs opacity-90 drop-shadow line-clamp-3">{videoData.description}</span>}
        ...
        {isCommentsOpen && (
          <div className="absolute inset-x-0 bottom-0 h-1/2 z-30 flex flex-col bg-white text-black rounded-t-lg shadow-[0_-4px_18px_rgba(0,0,0,0.4)] border-t border-black/10" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-black/10">
              ...
              <button onClick={onCloseComments} className="text-xs w-7 h-7 flex items-center justify-center rounded hover:bg-black/10" aria-label="Close comments">×</button>
            </div>
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 text-xs">
              {modalComments.length > 0 ? (
                modalComments.map((comment, commentIndex) => {
                  ...
                  const uniqueKey = (comment as any).id || `comment-${commentIndex}-${comment.content.substring(0, 10)}`;
                  return (
            ...
            <form onSubmit={e => handleCommentSubmit(e, id)} className="px-4 py-3 border-t border-black/10 flex gap-2">
              <input
                type="text"
                value={commentText}
