...
const VideoPage = () => {
  ...
  const feed: FeedVideo[] = useMemo(() => {
    if (!videosResponse?.data) return [];

    const raw = Array.isArray(videosResponse.data)
      ? videosResponse.data
      : Array.isArray(videosResponse.data.data)
    ...
    return raw
      .map((v: any, i: number) => {
        ...
        const vid = v.id || v._id || String(i);
        let comments: VideoComment[] | undefined;

        if (Array.isArray(v.comments)) {
          comments = v.comments.map((c: any, ci: number) => ({
            id: c.id || `${vid}-c${ci}`,
        ...
        return {
          ...
          author: v.author?.displayName || v.author?.handle || v.author || v.user?.username || v.user?.name || null,
  ...
  // Update audio level for all videos
  useEffect(() => {
  ...
  const handleCommentSubmit = (e: React.FormEvent<HTMLFormElement>, videoId: string) => {
    ...
    createCommentMutation.mutate({ 
      ...
      onSuccess: () => {
        refetchComments().catch((error) => {
          console.error('Failed to refetch comments after submission:', error);
        });
        setCommentText("");
      },
      onError: (error) => {
  ...
  return (
    <div
      className={`h-screen w-full ${isCommentsOpen ? "overflow-hidden" : "overflow-y-scroll"} snap-y snap-mandatory bg-black text-white`}
    >
      {feed.map((item, idx) => {
        const commentPreview = item.comments && item.comments.length > 0
          ? `${item.comments[0].author?.displayName || item.comments[0].author?.handle || ""}: ${item.comments[0].content}`
          : undefined;
            ...
            >
              ...
              <div className="absolute top-3 left-3 z-20 bg-black/50 hover:bg-black/70 transition-colors text-white text-xs px-3 py-2 rounded flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => {
                    e.stopPropagation();
                    setAudioLevel(Number(e.target.value));
                  }}
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 text-xs">
                    {commentsData && commentsData.length > 0 ? (
                      ...
                      <div className="text-xs opacity-50">
                        {commentsData === undefined ? 'Loading Comments...' : 'No Comments found.'}
                      </div>
                    )}
                  </div>
